...
| Full reads can flood context and allocate thousands of spans | Default full reads stop at 24,000 UTF-8 source bytes or 400 lines before building line spans or persisting a snapshot. `READ_TOO_LARGE` points to range/search and reports both the byte count and the line count, so the exceeded limit is visible. Range and full responses summarize disclosed IDs in `spans` and list each line in `lines` as `"r12 | body"`, keeping byte offsets in full evidence only. An explicit exact `expected_bytes` permits a larger full response; a mismatched count returns `READ_SIZE_CHANGED`. The 16 MiB source ceiling remains. Full evidence retrieval is still an explicit potentially large operation. |
| A failed preflight requires rebuilding the plan | `retry PLAN NEW_REQUEST_ID` and `ultra_edit_retry` reuse the exact candidate and original bases after the environment is corrected. New journals must prove no target write: completed failed preflight with no write intent, or every file recording `REPLACEMENT_FAILED` (the target was reobserved unchanged after a failed rename) or `NOT_ATTEMPTED`. A new plan and request preserve the original failure receipt. Existing `commit` replay stays idempotent; partial, unknown, and interrupted failures are ineligible. New journals also exclude post-preflight failures; older journals have the safe exception described below. |
...
| Search can disclose only matches 1–20 | Search accepts a zero-based match `offset` and returns `next_offset`. Supply the previous page's `snapshot` to use the same immutable source. Match IDs stay absolute, such as `m21`, and each continued page's snapshot retains the references disclosed before it, so one request against the last page's snapshot can edit every match paged through it under the one-entry-per-file rule. A different `query` drops the previous query's match references. Omitting the snapshot reads fresh bytes; `stale: true` marks a retained source that no longer matches the file. The target path must still resolve to the original file. |
