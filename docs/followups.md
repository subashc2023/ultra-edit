...
| Replace-all cannot handle self-overlapping patterns | `all` counts and replaces non-overlapping matches from left to right. `"aa"` in `"aaaa"` has two replacements; eight spaces have four replacements of `"  "`. `expected` is that replacement count. Search and unique `exact` checks continue counting overlapping starts to avoid concealing ambiguity. Conflicts between separate changes still reject the batch. |
...
2. **Overlapping counts:** overlapping discovery remains intentional for search
   ...
   the count of non-overlapping pairs.
